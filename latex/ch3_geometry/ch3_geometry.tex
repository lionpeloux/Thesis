...
\subsubsection{Discrete Curvature}
...
\begin{equation}
	\kappa_1 / \kappa_2(\alpha) = \kappa_1 / \kappa_2(1/\alpha)= \frac{1+\alpha}{(1+ \alpha^2 + 2 \alpha \, cos(\varphi_i))^{1/2}} \, cos^2(\varphi_i/2)
\end{equation}

\begin{figure}[t] 
\begin{center}
\includegraphics[]{courbure_3PT.pdf} 
\caption{Discrete curvature from 3-points circle.}
\label{fig:1_1}
\end{center}
\end{figure}
...
\begin{figure}[t] 
\begin{center}
\includegraphics[]{courbure_TAN.pdf} 
\caption{Discrete curvature from inner tangent circle.}
\label{fig:1_1}
\end{center}
\end{figure}
...
\begin{tikzpicture}
	...
	ytick={0, 2, 4, 6, 8,10} ]
		...
		{2*sin(deg(x)/2)};  \node at (axis cs:3.0,2.4) {$\kappa_1$};			
		...
		{\curvatureB(deg(x),1)};  \node at (axis cs:3.0,9.5) {$\kappa_2$};
	...
	ytick={0, 0.2, 0.4, 0.6, 0.8, 1}
		...
		{\curvatureRap(deg(x),1)};  \node at (axis cs:1.1,0.95) {$\kappa_1/\kappa_2$};
		\addplot[Tblue, samples = 500, domain = 0:3.1416, solid, very thin]
		{\curvatureRap(deg(x),2)}; 
%		\addplot[Tblue, samples = 500, domain = 0:3.1416, solid]
%		{\curvatureRap(deg(x),0.5)}; 
