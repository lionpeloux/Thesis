...
\item prise en compte des contraintes => méthode des multiplicateurs de lagrange
\end{enumerate}
...
Recall that the rod is supposed to be inextensible in Kirchhoff’s theory. Thus, there is no stretching energy associated with an axial strain. However, this constraint may be enforced via a penalty energy, which in practice is somehow very similar as considering an axial stiffness into the beam …
...
\end{equation}
Expressing $F_\epsilon$ on the basis $\tilde{F}_\epsilon$ gives for $\vect{u}_\epsilon$ :
\begin{equation}
	\begin{aligned}
