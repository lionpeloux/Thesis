...
\citef{Day1965} introduces the \emph{dynamic relaxation} method. He remarks that for a damped system the \blockquote{static equilibrium of a structure under a system of applied forces may be found by following the movement of the structure from its initial, un-deformed and unloaded, position until all vibrations resulting from its subsequent loading have died out}. He proposes to chose the damping factor to achieve critical damping, that is to obtained the fastest convergence to the steady state. He uses this computing method to study the non linear deformation of plan portal frame parametrized by two rotational degrees of freedom.
...
\begin{figure}[p]
	...
	\begin{fullpage}	
		...
		\subfloat[][Potential energy of the pendulum for the first steps.]{\includegraphics[width=120mm]{ch6_numeric/plot/1_pendulum/build_Ep.pdf}\label{fig:pendulum_Ep}}
		\vspace{10pt}
...
\begin{figure}[p]
     	\centering
	\begin{fullpage}	
		\subfloat[][Typical profile of the kinetic damping.]{\includegraphics[width=120mm]{ch6_numeric/plot/1_pendulum/build_Ek.pdf}\label{fig:pendulum_Ek}} \\
		\subfloat[][Convergence of the DR process in the phase space.]{\includegraphics[width=120mm]{ch6_numeric/plot/1_pendulum/build_Phase.pdf}\label{fig:pendulum_Phase}} \\
		\vspace{10pt}
...
Un effort a été fait, plus une preuve de concept qu'un vrai loigiciel.
Nous donnons ici les guide line pour une implémentation plus compète.

Séparer l'état d'un système (c'est à dire l'état de ses degrés de libertrés) avec sa description topologique (élements, propriéts).
Quid du chargement ?
...
\begin{figure}[p]
\begin{fullpage}
	\input{ch6_numeric/algo/algo_dr.tex}
\end{fullpage}
\end{figure}
\DecMargin{2em}
...
\begin{figure}[p]
\begin{fullpage}
	\input{ch6_numeric/algo/algo_run.tex}
\end{fullpage}
\end{figure}
\DecMargin{2em}
...
\subsubsection{Implementation guidlines}
