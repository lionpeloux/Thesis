...
\begin{alignat}{5}
	...
	=  \dot{\vect{x}}_i^{t-\Delta t / 2} 
	&&+ \Delta t  \cdot \frac{\vect{R}_i^{x,\,t}}{m_i^x}
	\\[0.5em]
	& \dot{\theta}_i^{t+\Delta t / 2} &&
	=  \dot{\theta}_i^{t-\Delta t / 2} 
	&&+ \Delta t  \cdot \frac{R_i^{\theta,\,t}}{m_i^{\theta}}
\end{alignat}
\end{subequations}
...
\begin{alignat}{5}
	& \vect{x}_i^{t+\Delta t} 
	&& =  \vect{x}_i^{t} + \Delta t \cdot \dot{\vect{x}}_i^{t-\Delta t / 2} 	
	&&+ {\Delta t}^2  \cdot \frac{\vect{R}_i^{x,\,t}}{m_i^{x}}
	\\[0.5em]
	& {\theta}_i^{t+\Delta t} 
	&& =  {\theta}_i^{t} + \Delta t  \cdot  \dot{{\theta}}_i^{t- \Delta t / 2}
	&&+ {\Delta t}^2  \cdot \frac{R_i^{\theta,\,t}}{m_i^{\theta}}
\end{alignat}
\end{subequations}
...
\begin{alignat}{5}
	...
	=  \frac{1}{2} \Delta t  \cdot \frac{\vect{R}_i^{x,\,t}}{m_i^x}
	\\[0.5em]
	& \dot{\theta}_i^{+\Delta t / 2} &&
	=  \frac{1}{2} \Delta t  \cdot \frac{R_i^{\theta,\,t}}{m_i^{\theta}}
\end{alignat}
\end{subequations}
...
The stability of the dynamic relaxation algorithm has been studied in many ways, depending the type of damping used. The main idea is to remind that the time step ($\Delta t$) and the mass of the particles ($m_i^x$, $m_i^{\theta}$) are parameters of the numerical integration. Therefore, because we are not interested by the motion itself but by the quasi-static response of the system when properly damped, the time step and the mass should be chosen~:
\begin{itemize}
