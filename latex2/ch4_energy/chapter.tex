...
Kirchhoff's theory of rods is presented thoroughly in the next chapter (see \cref{chp:kirchhoff}). In this chapter, although the assumptions are not exactly the ones made in  Kirchhoff's theory, we will stick to this denomination as introduced by \cite{Bergou2008}. In the present theory we will assume that~:
\begin{itemize}
\item the rod is inextensible,
\item cross-sections remain planar and normal to the centerline,
\item strains remain small so that the material deforms elastically.
\end{itemize}

\subsection{Geometric configuration}
The geometric configuration of the rod is described by its centerline $\vect{x}(s)$ and its cross-section. The centerline is a space curve parameterized by its arc length, denoted $s$. Cross-section orientations are followed along the centerline by a material frame $\{\vect{d}_3 (s),\vect{d}_1 (s),\vect{d}_2 (s)\}$ which is an adapted orthonormal moving frame aligned with the principal axes of inertia of the cross-section. 

Recall from \cref{sec:amf} that \emph{adapted} means $\vect{d}_3 (s)=\vect{x}'(s)=\vect{t}(s)$ is aligned with the tangent vector of the centerline. Her, the prime symbol denotes the derivation with repspect to the arc length parameter $s$. Recall also from \cref{sec:arclength_param} that $\norm{\vect{x}'(s)}=1$ because $\vect{x}$ is parametrized by arc length.

\subsection{Inextensibility assumption}
We assume that the rod is inextensible. As explained in \cite{Audoly2010}, based on a scaling argument, two cases arise for slender beams~: either the centerline stretches and the bending and twisting forces become negligible compared to the axial forces~; either the centerline remains inextensible. As we are not interested in the first case (that is dealing with beams used as cables in tension) we will assume that inextensibility assumption.

Remark that the previous description is only valid for inextensible rods in order to follow material points by their arc length indifferently in their rest or deformed configuration. As explained in~\cite{Audoly2010}, this hypothesis is usually relevant for slender beams. Indeed, in practice, if a slender member faces substantial axial strain the bending behaviour would become negligible due to the important difference between axial and bending stiffness. 

Here and thereafter, the length of the rod will be denoted $L$ and the arc length $s$ will vary (with no loss of generality) in $[0,L]$.
...
\begin{equation}
	m(s) = Q' - \kappa\vect{b} \cdot (\vect{d}_3 \times \mat{M})
\end{equation}
