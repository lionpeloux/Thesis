...
Each collar is itself composed of two hemicylindrical parts so that it can be opened to easily engage a tube. A M12 nut and a swivel T-bolt allow to lock the tube in the collar using friction. Collars are positioned over a \SI{1.5}{\mm} thick epdm ribbon wrapped around the tubes (see \cref{fig:swivel_dwg}).
...
\subsubsection{The interface layer}
This coupler is made to assemble two scaffold steel tubes together. Workers should tight strongly the collars of the coupler to ensure that the steel tubes won't slide in their collar. Here, it is clearly impossible to do that. Indeed the GFRP tube is to thin (only \SI{3.5}{\mm} thick) and tightening the collars to the maximum would damage it or even make it collapse.

However, preventing the connections to slide along the tubes is critical to maintain the in-plane shear degree of freedom of the grid. If connections would slide, the grid would probably not deploy in space as intended. The grid kinematic would be blocked at some points, developing high stresses that would lead to breakages.

To maintain a sufficient level of sliding resistance while preserving the material integrity it was decided to introduce an interface layer to~:
\begin{itemize}
...
Several materials were tested in different thickness. Somme of the results are presented in \cref{plot:layer_benchmark}. A \SI{1.5}{\mm} thick EPDM ribbon was found to be suitable for the design requirements of the project. This layer improves significantly the sliding resistance from about \SI{300}{\newton} to about \SI{1200}{\newton}.

The ribbons were ordered with a double sided tape (DST) face to facilitate their placement on the tubes. The influence of the temperature regarding the presence of the adhesive were investigated. The results show that after \SI{40}{\celsius} the scotch creeps and a loss of resistance occurs. A recent thermal study has demonstrated that with natural ventilation only, the temperature inside the structure could rise up to \SI{70}{\celsius}.\footnote{This study was done in jun 2017 by the design companies T/E/S/S and CHOULET for the reconfiguration project of the cathedral.}
...
\begin{fullpage}
	...
	\caption{Influence of the interface layer on the sliding resistance (clamping torque is \SI{20}{\newton\meter}.}
...
During the first two years and a half of its service life, the building has shown that the thermal comfort was far from ideal as the membrane has very poor thermal properties~:
\begin{itemize}
\item During winter, the confort is ensured during mass by a forced-air heating system positioned on the equipments slab, few meters away from the main structure. This solution is adapted to infrequent occupations of the building. In that case, the energetic cost remains limited even if the solution is far from optimal.
\item During summer, the temperature raises very fast when the sun shines. Masses are scheduled earlier. The forced-air system is used to ventilate the interior volume. But the comfort level is rapidly insufficient as the interior temperature quickly exceeds \SI{30}{\celsius}. The discomfort is amplified as the membrane gets very hot and radiates toward the inside.
\end{itemize}

At the time of writing, the building is being reconfigured and so its purpose. A better thermal comfort is now required and solutions have to be found. Thus, a study on the thermal behavior of the structure has been done. This study confirm what the experience has shown : the temperature can reach \SI{70}{\celsius} if not ventilated at all.  A serious mechanical ventilation can lower this maximum temperature 

\begin{figure}[p]
	\captionsetup[subfloat]{captionskip=10pt}
     	\centering
	\begin{leftfullpage}	
		\subfloat[][Temperature.]{\includegraphics[]{ch2a_creteil/plot/5_thermal/build_Text.pdf}\label{fig:T_1}} \\
		\subfloat[][Solar radiation.]{\includegraphics[]{ch2a_creteil/plot/5_thermal/build_Rad.pdf}\label{fig:T_2}} \\
		\vspace{10pt}
		\caption{Monthly temperature and solar radiation at the project location from 8:00 to 19:00.}
...
\begin{figure}[p]
	\captionsetup[subfloat]{captionskip=10pt}
     	\centering
	\begin{fullpage}	
		\subfloat[][Without ventilation.]{\includegraphics[]{ch2a_creteil/plot/5_thermal/build_Tint_noV.pdf}\label{fig:T_3}} \\
		\subfloat[][With ventilation.]{\includegraphics[]{ch2a_creteil/plot/5_thermal/build_Tint_V.pdf}\label{fig:T_4}} \\
		\vspace{10pt}
		\caption{Monthly interior temperature from 8:00 to 19:00.}
