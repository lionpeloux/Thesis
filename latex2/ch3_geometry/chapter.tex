...
The \emph{turning angle} is defined as the oriented angle between to adjacent edges : $\varphi_i=\angle(\vect{e}_{i-1},\vect{e}_i)$. It is defined only for all $i \in [1,n-1]$. It corresponds to the angle of rotation, in the osculating plane, around the binormal vector ($\vect{b}_i$), to align $\vect{e}_{i-1}$ with $\vect{e}_{i}$. The sign of $\varphi_i$ is taken in accordance to the right-hand rule regarding the orientation of $\vect{b}_i$. The next section will highlight the central role of the turning angle in the possible measurements of the discrete curvature.
...
\begin{equation}
	...
	t_i = i
\end{equation}
Thus, as in the smooth case, a discrete curve is said to be parametrized by arc length if $\norm{\Gamma'} = 1$, that is every edges are of unit length ($\norm{\vect{e}_i} = 1$). This constraint is sometimes relaxed to curves of constant edge length ($\norm{\vect{e}_i} = c$) that are said to be parametrized proportional to arc length.
...
By analogy with the smooth case, we define the curve arc length at vertices as :
\begin{equation}
	\left\{
	\begin{aligned}
		s_0 	&= 0 								& 	&i = 0		\\
		s_i 	&= \sum_{k=1}^i \norm{\vect{e}_{k-1}}		&	&i \in [\|1, n-1\|]	\\
		s_n 	&= \sum_{k=1}^{n} \norm{\vect{e}_{k-1}} = L 	&	&i = n		\\
	\end{aligned}
	\right.
\end{equation}
This definition naturally extends to the whole domain by piecewise linear interpolation. This is not different as considering the discrete curve as a continuous polygonal curve. Indeed, for any $s \in [s_i, s_{i+1}]$ there exist a normalized parameter $t = \frac{s - s_i}{s_{i+1} - s_i} \in [0,1]$ such that :
\begin{equation}
	\left\{
	\begin{aligned}
	...
	\right.
\end{equation}
Note that this parametrization satisfies $\norm{\Gamma'}=1$ on $\bigcup_{i=1}^n ]s_{i-1}, s_i[$ but $\Gamma'$ remains undefined at vertices. This issue is the reason why defining the tangent vector at vertices can not be done unequivocally.
...
\subsection{From osculating circles}

\begin{figure}[!t]
	\centering
%	\includegraphics[]{discrete_curve.pdf}
%	\caption{Discrete curve representation.}
%	\label{fig:discrete_curve}
%	\vspace{40pt}
	...
	\vspace{10pt}
	\caption{Four possibilities to define the osculating circle for discrete curves, leading to different notions of discrete curvature.}
	\label{fig:kb_def}
\end{figure}
...
\end{equation}
This has the disadvantage that it does not diverge at kinks and is limited to $[0,2]$, thus a proper condition on the discretization must be satisfied.
Problème de convergence lorsque l'angle tend vers pi et que les segments ont même longueur.
En pratique peut probable. Conduit à une limitation sur l'intégrale de courbure

% edge-based curvature
% ------------------------------
\subsubsection{Edge-based osculating circle}
Let $\Gamma$ be a discrete curve parametrized by arc length. The \emph{edge-based} (or inscribed) osculating circle at edge $\vect{e}_{i}$ is defined as the unique circle tangent to the edges $\vect{e}_{i-1}$, $\vect{e}_{i}$ and $\vect{e}_{i+1}$ (see \cref{fig:kb_edge}).
\begin{equation}
	\kappa_2 = \frac{\tan(\varphi_i/2) + \tan(\varphi_{i+1}/2)}{\norm{\vect{e}_{i}}}
\end{equation}
This definition shows an appropriate behavior when the curve tends to kicks, as the radius of curvature tends to zero ($\tan \pi/2 = \infty \Rightarrow \kappa = \infty$), and when the curve tends to be a straight line, as the curvature tends to $0$ ($\tan 0 = 0 \Rightarrow \kappa = 0$). However, it needs $\Gamma$ to be planar which is by far to restrictive regarding our purpose. Finally, this way of defining the curvature is not as local as one would expect as it is defined relatively to the edge $\vect{e}_{i}$ but not where the turning occurs, at vertices.

% mixed approach
% ---------------------
\subsubsection{Osculating circle for a curve parametrized by arc length}

The turning angle $\varphi_i$ is concentrated at vertex $\vect{x}_i$. This has the advantage that it diverge at kinks. The curvature is spread along the discrete curve from the middle of $\vect{e}_{i-1}$ to the middle of $\vect{e}_{i}$ :
\begin{equation}
	\vect{\kappa b} = \frac{2}{l} \left( \frac{\vect{e}_{i-1} \times \vect{e}_{i}}{\norm{\vect{e}_{i-1}} \norm{\vect{e}_{i}} + \vect{e}_{i-1} \cdot \vect{e}_{i}} \right)
	\quad , \quad
	\kappa_3 = \norm{\vect{\kappa b}} = \frac{2}{l} \tan(\varphi_i/2)
	\quad , \quad 
	l = \frac{\norm{\vect{e}_{i-1}} + \norm{\vect{e}_{i}}}{2}
\end{equation}

Qu'on peut réécrire pour des segments de longueur constante :
\begin{equation}
...
%\begin{figure}[t]
%	\centering
%	\subfloat[][]{\includegraphics{kb_anim_vertex_a.pdf}\label{fig:}}
%	\subfloat[][]{\includegraphics{kb_anim_vertex_b.pdf}\label{fig:}}\\
%	\subfloat[][]{\includegraphics{kb_anim_edge_a.pdf}\label{fig:}}
%	\subfloat[][]{\includegraphics{kb_anim_edge_b.pdf}\label{fig:}}
%	\vspace{10pt}
%	\caption{Benchmarking vertex-based and edge-based discrete curvatures.}
%	\label{fig:kb_bench}
%\end{figure}
...
\begin{figure}[t]
	...
	\subfloat[][vertex-based]{\includegraphics{kb_anim_vertex_b.pdf}\label{fig:2}} \\
	...
	\vspace{10pt}
	\caption{There is more than one way to define the osculating circle of a discrete curve.}
\end{figure}
...
\caption{Another definition of the osculating circle for curves parametrized by arc length.}
\label{fig:1_1}
\end{center}
\end{figure}
...
% ===================
\section{Discrete tangent vector}

\begin{figure}[t]
	\centering
	\includegraphics[]{kb_vertex_tangent.pdf}
	\caption{Tangent vector at verticies.}
	\label{fig:1_1}
\end{figure}
