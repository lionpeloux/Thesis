...
\begin{figure}[!p]
	...
	\vspace{10mm}
	\input{ch3_geometry/plot/bench_kb.tex}
...
%	\vspace{-5mm}
	...
	\vspace{10mm}
	\subfloat[][$| 1-\frac{\mathcal{E}_1}{\mathcal{E}}(n) |$ in \%]{\input{ch3_geometry/plot/bench_semicircle_vertex.tex}
		\label{plot:bench_semicircle_vertex}}\hspace{5mm}
	\subfloat[][$| 1-\frac{\mathcal{E}_3}{\mathcal{E}}(n) |$ in \%]{\input{ch3_geometry/plot/bench_semicircle_edge.tex}
...
%	\vspace{-5mm}
	...
	\label{fig:elastica}
	\vspace{10mm}
	\subfloat[][$|1-\frac{\mathcal{E}_1}{\mathcal{E}}(n) |$ in \%]{\input{ch3_geometry/plot/bench_elastica_vertex.tex}
		\label{plot:bench_elastica_vertex}}\hspace{5mm}
	\subfloat[][$| 1-\frac{\mathcal{E}_3}{\mathcal{E}}(n) |$ in \%]{\input{ch3_geometry/plot/bench_elastica_edge.tex}
...
\begin{figure}[!p]
	...
	\vspace{10pt}
	\caption{Parallel transport of vectors and frames from $\{\vect{x}_i,\vect{t}_i\}$ to $\{\vect{x}_{i+1},\vect{t}_{i+1}\}$.}
	\label{fig:pt}
\end{figure}
...
The double reflection method is given by \cite{Wang2008}. It is supposed to be of order $o(h^4)$ whereas the rotation method is only $o(h^2)$, where $h = \sup_{i\in \llbracket 0,1 \rrbracket} \norm{\vect{e}_i}$ is the sharpness of the discretization. Though its computation cost is quite similar, it is not subjected to instability when $\vect{t}_i$ and $\vect{t}_{i+1}$ tend to be collinear, which is an obvious advantage.

Let $\mathcal{R}_1$ be the reflection across the bisecting plane of $\vect{e}_i$. The unit vector $\vect{n}_1 = \vect{u}_i$ is normal to that plane. 
Let $\vect{t}_i^*$ be the image of $\vect{t}_i$ by $\mathcal{R}_1$ ($\vect{t}_i^* = \mathcal{R}_1(\vect{t}_i)$).
Let $\mathcal{R}_2$ be the reflection across the bisecting plane of the points $\vect{x}_{i+1} + \vect{t}_i^*$ and $\vect{x}_{i+1} + \vect{t}_{i+1}$. 
The unit vector $\vect{n}_2 = \frac{ \vect{t}_{i+1} - \vect{t}_i^*}{\norm{\vect{t}_{i+1} - \vect{t}_i^*}}$ is normal to that plane (see \cref{fig:pt_reflection}).

The parallel transport is defined as the \emph{double reflection} through $\mathcal{R}_1$ and $\mathcal{R}_2$ :
\begin{equation}
	\mathcal{P}_{(\vect{x}_{i},\vect{t}_{i})}^{(\vect{x}_{i+1},\vect{t}_{i+1})} = \mathcal{P}_i^{i+1} = \mathcal{R}_2 \circ \mathcal{R}_1
\end{equation}
Let $\mathcal{F}_i = \{ \vect{t}_i, \vect{d}_1, \vect{d}_2 \}$ be a direct orthonormal frame at $\vect{x}_i$. 
Let $\mathcal{F}_i^* = \{ \vect{t}_i^*, \vect{d}_1^*, \vect{d}_2^* \}$ be the image of $\mathcal{F}_i$ by $\mathcal{R}_1$ : $\mathcal{F}_i^* = \mathcal{R}_1 (\mathcal{F}_i)$.
Then : 
\begin{subequations}
\begin{alignat}{3}
...
\end{alignat}
\end{subequations}
Let $\mathcal{F}_i^{\parallel} = \{ \vect{t}_{i+1}, \vect{d}_1^{\parallel}, \vect{d}_2^{\parallel} \}$ be the image of $\mathcal{F}_i^*$ by $\mathcal{R}_2$ : $\mathcal{F}_i^{\parallel} = \mathcal{R}_2 (\mathcal{F}_i^*) = \mathcal{P}(\mathcal{F}_i)$. 
Then the parallel transported vectors are :
\begin{subequations}
\begin{alignat}{3}
