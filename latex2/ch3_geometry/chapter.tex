...
A discrete (or polygonal) space curve is defined as a finite sequence of $n+1$ points of $\mathbb{R}^3$ ($\vect{x}_0,  \vect{x}_1, \ldots, \vect{x}_n$) called \emph{vertices} (see \cref{fig:discrete_curve}). Pairs of adjacent vertices define $n$ straight segments ($\vect{e}_0,  \vect{e}_1, \ldots, \vect{e}_{n-1}$) called \emph{edges}. Each edge is of length $l_i = \norm{\vect{e}_i}$.

\subsubsection{Turning angle}
The angle between to adjacent edges is called the \emph{turning angle} : $\varphi_i = \angle(\vect{e}_{i-1},\vect{e}_i) \; , \; \forall i \in [1,n-1]$.
...
The curve arc-length is defined as : $\vect{x}_i = \vect{x}(s_i)$ with $s_i = \sum_{k=1}^i l_k$. Thus the length of the curve is :
\begin{equation}
	L = \sum_{i=0}^n \norm{\vect{e}_i}
\end{equation}

A discrete curve $\gamma$ is called parameterized by arc-length if $\norm{\gamma}=cst$. is called regular if any three successive points are pairwise disjoined.
...
Plusieurs approches : le cercle osculateur ou la variation de longueur par transformation parallèle.
...
Known as the \emph{Menger curvature}, it is defined as the inverse of the radius of the circle passing through three consecutive vertices.
\begin{equation}
...
% ------------------------------
\subsection{Edge-based osculating circle}
...
The turning angle $\varphi_i$ is concentrated at vertex $\vect{x}_i$. The curvature is spread along the discrete curve from the middle of $\vect{e}_{i-1}$ to the middle of $\vect{e}_{i}$ :
\begin{equation}
