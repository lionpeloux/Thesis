...
\cite{Hanson95}
The notion of \emph{parallel transport} is somehow a generalization of the classical notion of collinearity in flat euclidean spaces (e.g.\ $\mathbb{R}^2$ or $\mathbb{R}^3$ spaces) to spaces that exhibit some non vanishing curvature (e.g.\ spheric or hyperbolic spaces).

Following \cite{Bishop1975}, we define a \emph{(relatively) parallel field}. Let $\gamma$ be a regular arc-length parametrized curve. Let $\vect{p}$ be a vector field along $\gamma$. The vector field $\vect{p}$ si said to be \emph{parallel} if its derivative is purely tangential, that is :
\begin{equation}
...
} $\vect{e}_1$ and $\vect{e}_2$ are both \emph{parallels} if and only if the frame angular velocity is itself a normal field, that is :
\begin{equation}
	\vect{\Omega}(s) \cdot \vect{t}(s) = 0 \Leftrightarrow \tau(s) = 0
\end{equation}
Reciprocally, a vector $\vect{p}(0)$ can be \emph{parallel transported} along the curve if it is propagated at angular speed 
as a vector field 
...
A \emph{Bishop frame} denoted $\{\vect{t},\vect{u},\vect{v}\}$, also known as \emph{zero-twisting} or \emph{parallel-transported} frame, is an adapted moving frame that has no tangential angular velocity :
\begin{equation}
...
\begin{equation}
	...
	= \vect{\kappa b}(s) 
\end{equation}
Consequently, the Bishop frame satisfies $\vect{\Omega_b}(s) \cdot \vect{t}(s) = 0$ and is thus \emph{rotation-miniminzing} regarding the tangent vector. The motion of this frame through the curve is known as \textquote{roll-free}.
...
\blockcquote{Bloomenthal1990}{The mechanism to minimize torsion defines an initial reference frame at the beginning of the curve; this frame is propagated along the curve using small, local rotations. This method is immune to degeneracies in the curvature vector \textelp{}}
...
As a consequence, Frenet and Bishop frames coincide within a constant rotation around the unit tangent vector for planar curves ($\tau_f = 0$).
