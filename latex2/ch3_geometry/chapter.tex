...
These quantities, also known as \textquote{generalized curvatures}, are essential to understand the geometry of space curves. As stated by the \emph{Fundamental Theorem of Space Curves} \footnote{The full demonstration of this theorem is attributed to Darboux in \cite[p.11]{Delcourt2007}.}, a curve is fully determined by its curvature and torsion up to a solid movement in space.
...
%Curvature
\subsection{First invariant : curvature}\label{sec:curvature}
...
The \emph{torsion} is a scalar quantity defined at any given parameter $s \in J$ as :
\begin{equation}
...
Remark that here the torsion is denoted \textquote{$\tau_f$} and not simply \textquote{$\tau$} as the latter will be reserved to denote any angular velocity of a moving adapted frame around its tangent vector. Thus, $\tau_f$ refers to the particular angular velocity of the Frenet trihedron around it's tangent vector. This torsion, which is a geometric property of the curve, will be indifferently referred to as the \emph{torsion of Frenet} or the \emph{geometric torsion}.
...
This is easily understood by expressing the first order differential equation satisfied by the Frenet trihedron $\{\vect{t},\vect{n},\vect{b}\}$ and known as the \emph{Serret-Frenet formulas} :
\begin{equation}
	\left\{
	\begin{aligned}
...
While the Frenet trihedron is 

In his paper \textquote{There is more than one way to frame a curve}, Bishop remarks that \blockcquote[p.1]{\Bishop1975}{the Frenet frame has long been the standard vehicle for analysing properties of the curve invariant under euclidean motions}.

introduces the differential equation that any arbitrary moving frame would satisfy. He states that this 
