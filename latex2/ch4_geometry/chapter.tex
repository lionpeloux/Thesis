...
Although the theoretical beam model takes place in the smooth world, our model will be implemented in a numerical solver, hence the necessity of a discrete representation. However, the two worlds are intimately related to each other and this is why we chose to present them both in this chapter.\footnote{\citef[preface]{LHospital1696}~: \blockquote{Car les courbes n’étant que des polygones d’une infinité de côtés, \& ne différant
...
\citef{Bishop1975} remarks that the usual Frenet frame is not the only way to frame a curve. He gives the skew-symmetric system of differential equations that any moving frame satisfies. He remarks that this system is governed by only three coefficient entries, which represent the components of the angular velocity vector of the frame expressed on the frame axes. He argues that the Frenet frame gains part of its significance because it is adapted to the curve and because one component of its angular velocity is null. Hence, he looks for other kind of moving frames that are both adapted and with one of the components of the angular velocity vector that is null. In particular, he looks at adapted frames that does not turn around the curve~: what will be called a Bishop frame hereafter.
...
\citef{Wang2008} introduce the double reflexion method to propagate rotation minimizing frames. This method is supposed to be more stable than the rotation method. \citef{Farouki2014} investigate the use of rotation-minimizing frames that minimize the rotation around the binormal vector of the curve (compared to Bishop frame that minimizes the rotation around the tangent vector of the curve).
...
\afterpage{%
	\AddToShipoutPictureBG*{% 
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{}{Definition of the tangent vector and the osculating circle of a curve}{fig:tangent_def}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=Abr,
			anchor=north east,
			xshift=-0.5,
			yshift=0.25cm,
			]{subfigure}{}{Curve's tangent}{fig:tangent_def_a}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Bbr,
			anchor=north east,
			yshift=0.25cm,
			]{subfigure}{}{Curve's normal and osculating circle}{fig:tangent_def_b}
		...
		\PhotoTextBox[
			...
			border=false,
				...
				\figurecaption{fig:tangent_def}\par
				\figurecaption{fig:tangent_def_a}\par
				\figurecaption{fig:tangent_def_b}
			}
	}
	\hbox{\vspace{6.75cm}}
}
...
\afterpage{%
	\AddToShipoutPictureBG*{% 
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{}{Osculating circles for a spiral curve at different parameters}{fig:spiral}
		\intersectnode{CAtl |- Abr}{Pt}
		\PhotoTextBox[
			node=Pt,
			anchor=north west,
			...
			border=false,
				...
				\figurecaption{fig:spiral}\par
			}
	}
	\setlength{\tmpheight}{8cm + 2\PhotoBigSkip}
...
\afterpage{%
	\AddToShipoutPictureBG*{% 
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{Discontinuity of the Frenet trihedron at an inflexion point}{Discontinuity of the Frenet trihedron at an inflexion point where the curvature vanishes and the orientation of the osculating plane is subject to a jump of angle $\pi/2$.}{fig:torsion_jump}
		\intersectnode{CAtl |- Abr}{Pt}
		\PhotoTextBox[
			node=Pt,
			anchor=north west,
			...
			border=false,
				...
				\figurecaption{fig:torsion_jump}
			}
	}
	\setlength{\tmpheight}{10cm + 2\PhotoBigSkip}
	\hbox{\vspace{\tmpheight}}
}
...
Finally, these formulas can be generalized to any non unit-speed parametrization of a curve.\footnote{See~\cite[p.203]{Gray2006} for a complete proof.} Let $\gamma = (I,F)$ be a biregular parametric curve. Let $t \in I$ be a parameter. Then the following \emph{generalized Serret-Frenet formulas} hold~:
\begin{subequations}
	\begin{alignat}{2}
...
\blankpage{
	...
	\AddToShipoutPictureBG*{% 
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{}{Geometric interpretation of the angular velocity vector of a moving frame}{fig:geo_interpretation}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Btl,
			anchor=north west,
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{Adapted moving frame on a circular helix}{Moving frame $F(s) =\{\vect{e}_{3}(s),\vect{e}_{1}(s),\vect{e}_{2}(s)\}$ on a circular helix. The frame is adapted as $\vect{e}_3(s) = \vect{t}(s)$.}{fig:adapted}
		\intersectnode{CAtl |- Abr}{Pt}
		\PhotoTextBox[
			node=Pt,
			anchor=north west,
			...
			border=false,
				...
				\figurecaption{fig:geo_interpretation}
			}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
...
One benefit of the Frenet frame is that, when transported along a \emph{closed curve}, the frame at the end of the curve will align back with the frame at the beginning of the curve, that is the frame will return to its initial value after a complete turn. During its trip, the frame will make a total twist of $\int_0^L \tau_f(s)ds = 0[2\pi]$ around the tangent vector.
...
\doubleblankpage[l]{
	...
	\AddToShipoutPictureBG*{% 
		\Photo[
			node=CAc,
			anchor=center,
			]{ch4_geometry/plot/1_bench_frame/build_frenet_large.pdf}%
		\savenodes{A}
		%%
		\PhotoCaptionRef[
			hrefnode=Atl,
			...
			anchor=north west,
			phantom=true,
			]{figure}{}{Angular velocities of Frenet and Bishop frames for a circular helix}{fig:frame_bench}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=Atr,
			anchor=north east,
			xshift=-1.0cm,
			yshift=-1.5cm,
			]{subfigure}{}{Frenet frame}{plot:frenet}
	}
}{
	\hbox{}\thispagestyle{empty}
	\AddToShipoutPictureBG*{% 
		\Photo[
			node=CAc,
			anchor=center,
			]{ch4_geometry/plot/1_bench_frame/build_bishop_large.pdf}%
		\savenodes{A}
		%%
		\PhotoCaptionRef[
			hrefnode=Atl,
			...
			xshift=-1.0cm,
			yshift=-1.5cm,
			]{subfigure}{}{Bishop Frame}{plot:bishop}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			width=\textwidth,
			border=false,
				...
				\figurecaption{fig:frame_bench}\par
				\figurecaption{plot:frenet}\par
				\figurecaption{plot:bishop}
			}
	}
}
...
% \centering
% 	\vspace{-0.5cm}
% 	\subfloat[][Frenet frame]{\includegraphics[]{ch4_geometry/plot/1_bench_frame/build_frenet_large.pdf}\label{plot:frenet}}
% 	\\
% %  	 \hspace{0.2cm}
% 	\subfloat[][Bishop frame]{\includegraphics[]{ch4_geometry/plot/1_bench_frame/build_bishop_large.pdf}\label{plot:bishop}}
...
\afterpage{%
	\AddToShipoutPictureBG*{% 
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{}{Discrete curve representation and parametrization}{fig:discrete_curve}
		\intersectnode{CAtl |- Abr}{Pt}
		\PhotoTextBox[
			node=Pt,
			anchor=north west,
			...
			border=false,
				...
				\figurecaption{fig:discrete_curve}
			}
	}
	\setlength{\tmpheight}{7cm + 0\PhotoBigSkip}
	\hbox{\vspace{\tmpheight}}
}
...
\doubleblankpage[r]{%
	...
	\AddToShipoutPictureBG*{% 
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{}{Several ways to define the osculating circle for discrete curves}{fig:kb_def}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=Ab,
			anchor=north,
			% yshift=\PhotoRefSkip,
			]{subfigure}{}{Vertex-based}{fig:kb_vertex}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Bb,
			anchor=north,
			]{subfigure}{}{Edge-based}{fig:kb_edge}
		\PhotoCaptionRef[
			hrefnode=Ctl,
			node=Cb,
			anchor=north,
			]{subfigure}{}{Bitangent with $\norm{\vect{e}_{i-1}}$}{fig:kb_mixed_a}
		\PhotoCaptionRef[
			hrefnode=Dtl,
			node=Db,
			anchor=north,
			]{subfigure}{}{Bitangent with $\norm{\vect{e}_{i}}$}{fig:kb_mixed_b}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			width=\textwidth,
			border=false,
				...
				\figurecaption{fig:kb_def}\par
				\figurecaption{fig:kb_vertex}\par
				\figurecaption{fig:kb_edge}\par
				\figurecaption{fig:kb_mixed_a}\par
				\figurecaption{fig:kb_mixed_b}
			}
	}
}{
	\hbox{}\thispagestyle{empty}
	\AddToShipoutPictureBG*{%
		\SBox[%
			...
			anchor=center,
				...
				\begin{tabularx}{\textwidth}{@{} X c c c r r r @{}}
					\toprule
		...
		\PhotoCaptionRef[
			...
			% yshift=-\PhotoRefSkip,
			phantom=true,
			]{table}{Review of several discrete curvature definitions}{Review of several discrete curvature definitions mentioned in the literature}{tab:kb_def}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			width=0.5\textwidth,
...
In particular,~\citet{Vouga2014} details which discrete curvature definition  faithfully transposes which property of the smooth curvature. He remarks that there is no \textquote{free-lunch} as none of the proposed definitions satisfies every properties of the smooth curvature.
...
\doubleblankpage[l]{%
	...
	\AddToShipoutPictureBG*{% 
		...
		\Photo[
			...
			anchor=north east,
			xshift=-\PhotoBigSkip/2,
			]{kb_anim_vertex_a.pdf}%
		\savenodes{A}
		\Photo[
			node=CAt,
			anchor=north west,
			xshift=\PhotoBigSkip/2,
			]{kb_anim_vertex_b.pdf}%
		\savenodes{B}
		\intersectnode{CAt |- Abr}{Pt}
		\Photo[
			node=Pt,
			anchor=north east,
			xshift=-\PhotoBigSkip/2,
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{Sensitivity of discrete curvatures to non uniform discretization}{Comparison of circumscribed and inscribed osculating circles for different values of the turning angle ($\varphi$)}{fig:kb_animation}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=Ab,
			anchor=north,
			% yshift=\PhotoRefSkip,
			]{subfigure}{}{Circumscribed ($\varphi=\pi/3$)}{fig:kb_anim_vertex_a}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Bb,
			anchor=north,
			]{subfigure}{}{Circumscribed ($\varphi=2\pi/3$)}{fig:kb_anim_vertex_b}
		\PhotoCaptionRef[
			hrefnode=Ctl,
			node=Cb,
			anchor=north,
			]{subfigure}{}{Inscribed ($\varphi=\pi/3$)}{fig:kb_anim_edge_a}
		\PhotoCaptionRef[
			hrefnode=Dtl,
			node=Db,
			anchor=north,
			]{subfigure}{}{Inscribed ($\varphi=2\pi/3$)}{fig:kb_anim_edge_b}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			% width=\textwidth,
			border=false,
				...
				\figurecaption{fig:kb_animation}\par\medskip
				\figurecaption{fig:kb_anim_vertex_a}\par
				\figurecaption{fig:kb_anim_vertex_b}\par
				\figurecaption{fig:kb_anim_edge_a}\par
				\figurecaption{fig:kb_anim_edge_b}
			}
	}
}{%
	\hbox{}\thispagestyle{empty}
	\AddToShipoutPictureBG*{%
		\Photo[
			node=CAc,
			anchor=center,
			]{ch4_geometry/plot/2_bench_kb/build.pdf}%
		\savenodes{A}
		%
		\PhotoCaptionRef[
			hrefnode=Atl,
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{Sensitivity of discrete curvatures to non uniform discretization}{Comparison of circumscribed and inscribed osculating circles for different values of the turning angle ($\varphi$)}{fig:kb_bench}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			% width=\textwidth,
...
However, for the kind of structures we are studying here, those kind of configurations are not likely to arise. And if they do, the structure would be severely damaged and this situation is to be avoided by the designers. Moreover, the sharpness of the discretization could be increased to reduce the value of the turning angles and stay in the range $[0,\pi/4]$ where the circumscribed curvature gives accurate results.
...
\doubleblankpage[l]{%
	...
	\AddToShipoutPictureBG*{%
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{}{Discretization of a semicircle and evaluation of its bending energy}{fig:semicircle}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			% width=\textwidth,
	...
	\AddToShipoutPictureBG*{% 
		...
		\Photo[
			...
			xshift=-\PhotoBigSkip/2,
			% gopt={width=\tmpwidth},
			]{ch4_geometry/plot/3_bench_semicircle/build_vertex.pdf}%
		\savenodes{A}
		\Photo[
			node=CAc,
			anchor=west,
			xshift=\PhotoBigSkip/2,
			% gopt={width=\tmpwidth},
			]{ch4_geometry/plot/3_bench_semicircle/build_edge.pdf}%
		\savenodes{B}
		%
		\PhotoCaptionRef[
			hrefnode=Atl,
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{Relative error in the estimation of the bending energy of a semicircle}{Relative error in the estimation of the bending energy of a semicircle ($\mathcal{E}$) by the discrete energies $\mathcal{E}_1$ and $\mathcal{E}_3$, regarding the sharpness of the discretization.}{fig:bench_semicircle}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=At,
			anchor=south,
			% yshift=\PhotoRefSkip,
			]{subfigure}{}{$| 1-\frac{\mathcal{E}_1}{\mathcal{E}}(n) |$ in \%}{plot:bench_semicircle_vertex}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Bt,
			anchor=south,
			]{subfigure}{}{$| 1-\frac{\mathcal{E}_3}{\mathcal{E}}(n) |$ in \%}{plot:bench_semicircle_edge}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			% width=\textwidth,
			border=false,
				...
				\figurecaption{fig:bench_semicircle}\par\medskip
				\figurecaption{plot:bench_semicircle_vertex}\par
				\figurecaption{plot:bench_semicircle_edge}
			}
	}
}
...
\doubleblankpage[l]{%
	...
	\AddToShipoutPictureBG*{%
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{}{Discretization of an elastica curve and evaluation of its bending energy}{fig:elastica}
		\intersectnode{Atr -| CAtr}{Pt}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=Pt,
			anchor=north east,
			% yshift=\PhotoRefSkip,
			]{subfigure}{}{Sequence of elastica curves}{fig:elastica_seq}
		\intersectnode{Btr -| CAtr}{Pt}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Pt,
			anchor=north east,
			% yshift=\PhotoRefSkip,
			]{subfigure}{}{Zoom on the discretization}{fig:elastica_discretization}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			% width=\textwidth,
			border=false,
				...
				\figurecaption{fig:elastica}\par\medskip
				\figurecaption{fig:elastica_seq}\par
				\figurecaption{fig:elastica_discretization}
			}
	}
}{%
	\hbox{}\thispagestyle{empty}
	\AddToShipoutPictureBG*{% 
		...
		\Photo[
			...
			xshift=-\PhotoBigSkip/2,
			% gopt={width=\tmpwidth},
			]{ch4_geometry/plot/4_bench_elastica/build_vertex.pdf}%
		\savenodes{A}
		\Photo[
			node=CAc,
			anchor=west,
			xshift=\PhotoBigSkip/2,
			% gopt={width=\tmpwidth},
			]{ch4_geometry/plot/4_bench_elastica/build_edge.pdf}%
		\savenodes{B}
		%
		\PhotoCaptionRef[
			hrefnode=Atl,
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{Relative error in the estimation of the bending energy of an elastica}{Relative error in the estimation of the bending energy of an elastica ($\mathcal{E}$) by the discrete energies $\mathcal{E}_1$ and $\mathcal{E}_3$, regarding the sharpness of the discretization. The curves (1,5,10,15,20,25,30) are chosen from \cref{fig:elastica_seq}.}{fig:bench_elastica}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=At,
			anchor=south,
			% yshift=\PhotoRefSkip,
			]{subfigure}{}{$| 1-\frac{\mathcal{E}_1}{\mathcal{E}}(n) |$ in \%}{plot:bench_elastica_vertex}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Bt,
			anchor=south,
			]{subfigure}{}{$| 1-\frac{\mathcal{E}_3}{\mathcal{E}}(n) |$ in \%}{plot:bench_elastica_edge}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			% width=\textwidth,
			border=false,
				...
				\figurecaption{fig:bench_elastica}\par\medskip
				\figurecaption{plot:bench_elastica_vertex}\par
				\figurecaption{plot:bench_elastica_edge}
			}
	}
}
...
\blankpage{%
	...
	\AddToShipoutPictureBG*{%
		\Photo[
			...
			anchor=north,
			yshift=2\PhotoBigSkip,
			]{kb_vertex_tangent.pdf}%
		\savenodes{A}
		\intersectnode{Abr -| CAc}{Pt}
		\Photo[
			node=Pt,
			anchor=north east,
			xshift=-\PhotoBigSkip/2,
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{Definition of the tangent vector associated to the circumscribed curvature}{Definition of the tangent vector ($\vect{t}$) and related curvature binormal vector ($\vect{\kappa b}$) at vertices associated to the circumscribed curvature}{fig:kb_vertex_tangent}
		\intersectnode{Atr -| CAtr}{Pt}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=Pt,
			anchor=north east,
			yshift=-2\PhotoBigSkip,
			]{subfigure}{}{Current portion}{fig:kb_vertex_tangent_a}
		\intersectnode{Btr -| CAtl}{Pt}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Pt,
			anchor=north west,
			yshift=-\PhotoBigSkip,
			]{subfigure}{}{Start}{fig:kb_vertex_tangent_b}
		\intersectnode{Ctr -| CAtr}{Pt}
		\PhotoCaptionRef[
			hrefnode=Ctl,
			node=Pt,
			anchor=north east,
			yshift=-\PhotoBigSkip,
			]{subfigure}{}{End}{fig:kb_vertex_tangent_c}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			width=0.6\textwidth,
			border=false,
				...
				\figurecaption{fig:kb_vertex_tangent}\par\medskip
				\figurecaption{fig:kb_vertex_tangent_a}\par
				\figurecaption{fig:kb_vertex_tangent_b}\par
				\figurecaption{fig:kb_vertex_tangent_c}
			}
	}
}
...
\blankpage{%
	...
	\AddToShipoutPictureBG*{%
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{Definition of the tangent vector associated to the inscribed curvature}{Definition of the tangent vector ($\vect{t}$) and related curvature binormal vector ($\vect{\kappa b}$) at vertices associated to the inscribed curvature}{fig:kb_edge_tangent}
		\intersectnode{Atr -| CAtr}{Pt}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=Pt,
			anchor=north east,
			yshift=-2\PhotoBigSkip,
			]{subfigure}{}{Current portion}{fig:kb_edge_tangent_a}
		\intersectnode{Btr -| CAtl}{Pt}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Pt,
			anchor=north west,
			yshift=-\PhotoBigSkip,
			]{subfigure}{}{Start}{fig:kb_edge_tangent_b}
		\intersectnode{Ctr -| CAtr}{Pt}
		\PhotoCaptionRef[
			hrefnode=Ctl,
			node=Pt,
			anchor=north east,
			yshift=-\PhotoBigSkip,
			]{subfigure}{}{End}{fig:kb_edge_tangent_c}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			width=0.6\textwidth,
			border=false,
				...
				\figurecaption{fig:kb_edge_tangent}\par\medskip
				\figurecaption{fig:kb_edge_tangent_a}\par
				\figurecaption{fig:kb_edge_tangent_b}\par
				\figurecaption{fig:kb_edge_tangent_c}
			}
	}
}
...
Though these points does not generally fall into mid-edge, the tangent vector can also be identified to $\vect{u}_{i-1}$ (resp. $\vect{u}_{i}$) at point $\tilde{\vect{x}}_i^- = \vect{x}_i - \frac{1}{2}\rconf{l_i} \vect{u}_{i-1}$ (resp. $\tilde{\vect{x}}_i^+ = \vect{x}_i + \frac{1}{2}\rconf{l_i} \vect{u}_{i}$)~:
\begin{subequations}
\setlength{\jot}{6pt}
\begin{alignat}{4}
...
\blankpage{%
	...
	\AddToShipoutPictureBG*{%
		...
		\PhotoCaptionRef[
			...
			yshift=\PhotoRefSkip,
			phantom=true,
			]{figure}{Two methods to parallel transport a vector}{Two methods to parallel transport a frame from $\{\vect{x}_i,\vect{t}_i\}$ to $\{\vect{x}_{i+1},\vect{t}_{i+1}\}$}{fig:pt}
		\PhotoCaptionRef[
			hrefnode=Atl,
			node=CAtr,
			anchor=north east,
			% yshift=-2\PhotoBigSkip,
			]{subfigure}{}{Rotation method}{fig:pt_rotation}
		\intersectnode{Btl -| CAtr}{Pt}
		\PhotoCaptionRef[
			hrefnode=Btl,
			node=Pt,
			anchor=north east,
			yshift=-2\PhotoBigSkip,
			]{subfigure}{}{Double reflection method}{fig:pt_reflection}
		\PhotoTextBox[
			node=CAbl,
			anchor=base west,
			width=\textwidth,
			border=false,
				...
				\figurecaption{fig:pt}\par
				\figurecaption{fig:pt_rotation}\par
				\figurecaption{fig:pt_reflection}\par
			}
	}
}
...
% 	\vspace{10pt}
% 	\captionof{figure}[Two methods to parallel transport a vector]{Two methods to parallel transport a frame from $\{\vect{x}_i,\vect{t}_i\}$ to $\{\vect{x}_{i+1},\vect{t}_{i+1}\}$.}
% 	\label{fig:pt}
% \end{figure}
