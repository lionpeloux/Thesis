...
\setlength{\BackTitleWidth}{14.3cm}
...
\AddToShipoutPictureBG*{%
	...
	\begin{tikzpicture}[remember picture, overlay, inner sep=0pt]
		\path (PGtl) -- (PGtl -| SPtl) coordinate[midway] (Pt);
		\coordinate[xshift=-\BackTitleWidth/2, yshift=-\BackTitleTopMargin] (HA) at (Pt);
		\node[anchor=base west, yshift=0cm] at (HA){%
			\tfHA%
			\fbox{\parbox[b]{\BackTitleWidth}{\spaceout{Modeling of bending-torsion couplings in active-bending structures}}}%
		};
		...
		\node[anchor=base west, yshift=0cm] at (HB){%
			\tfHB%
			\fbox{\parbox[b]{\BackTitleWidth}{\spaceout{APPLICATION TO THE DESIGN OF ELASTIC GRIDSHELLS}}}%
		};
		% abstract
		\setlength\columnsep{0.5cm}%
		\coordinate[yshift=8.15cm] (HC) at (HA |- MBphd);
		% \coordinate (HC) at (HC);
		\path[fill,black] (PPtl |- HC) rectangle ([xshift=-\BackTickMargin, yshift=-8mm]HC);
		\node[anchor=north west] at (HC){%
			\fbox{\parbox[t]{\BackTitleWidth}{%
			\tftxt\setstretch{1.0}%
			...
			\end{multicols}}}};
		\coordinate (HC) at (MBphd -| Pt);
		\node[anchor=center] at (HC){%
			\fbox{\parbox[c]{\BackTitleWidth}{%
