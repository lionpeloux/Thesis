...
\let\defaultpgflinewidth\pgflinewidt
...
\AddToShipoutPictureBG*{%
	\begin{tikzpicture}[remember picture, overlay, inner sep=0pt]
		% background color
		\path[fill,gray1] (PPbl) rectangle (PPtr);
		% main bar nodes
...
% \clearpage
% %http://tug.ctan.org/tex-archive/macros/latex/contrib/wrapfig/multiple-span.txt
% \parbox[t]{\CoverWidth-2\BackTitleSideMargin}{%
% 	\tftxt\setstretch{1.0}%
...
% 	\begin{multicols}{2}
% 		\noindent
% 		An \emph{elastic gridshell} is a freeform structure, generally doubly curved, but formed out through the reversible deformation of a regular and initially flat structural grid. Building curved shapes that may seems to offer the best of both worlds~: shell structures are amongst the most performant mechanically speaking while planar and orthogonal constructions are much more efficient and economic to produce than curved ones. This ability to \textquote{form a form}\begin{wrapfigure}[9]{r}[\dimexpr.5\width+.5\columnsep\relax]{6cm}%
% 			\includegraphics[width=6cm,height=8\baselineskip]{example-image-a}%
% 		\end{wrapfigure}
% 		efficiently is of peculiar importance in the current context where morphology is a predominant component of modern architecture, and envelopes appear to be the neuralgic point for building performances.
