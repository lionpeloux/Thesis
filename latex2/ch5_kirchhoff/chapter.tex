...
\begin{itemize}
\item The shear force : $\vect{F} = F_1 \vect{d}_1 + F_2 \vect{d}_2$ 
...
\end{equation}
We also introduce the matrix notation for the derivatives of the internal forces and moments referring to the inertial frame of reference $\{\vect{x},\vect{x},\vect{z}\}$ and to the local frame $\{\vect{d}_{1},\vect{d}_{2},\vect{d}_{3}\}$. The (spatial) velocity of $\{\vect{d}_{1},\vect{d}_{2},\vect{d}_{3}\}$ implies that~:
\begin{subequations}
	\begin{alignat}{3}
	&\vect{F}' = \vect{F}_{xyz}' = \vect{F}_{123}' + \vect{\varkappa} \times \vect{F}
	\\
	&\vect{M}' = \vect{M}_{xyz}' = \vect{M}_{123}' + \vect{\varkappa} \times \vect{M}
	\end{alignat}
\end{subequations}
...
\begin{subequations}
	\begin{alignat}{1}
	\vect{N}_{2i+1/2} &= N_{2i+1/2}\, \vect{d}_{3,2i+1/2} \quad \text{where} \quad {N_{i+1/2}} = ES_i \,\epsilon_{2i+1/2}
	\\
	\vect{N}_{2i+3/2} &= N_{2i+3/2}\, \vect{d}_{3,2i+3/2}\quad \text{where} \quad {N_{i+3/2}} = ES_i \,\epsilon_{2i+3/2}
	\end{alignat}
\end{subequations}
Remark the sign convention~: as expected, when edge $\vect{e}_i$ suffers a positive extension ($\epsilon_{i+1/2} > 0$), vertex $\vect{x}_i$ \textquote{attracts} vertex $\vect{x}_{i-1}$ to it as $\vect{d}_{3,i+1/2}$ is pointing from $\vect{x}_{i}$ towards $\vect{x}_{i+1}$. Remark also that $\epsilon_{i+1/2} = 0 \Leftrightarrow l_i = \rconf{l}_i$ when the rod is not stretched.
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
To compute $\Delta\theta_{i} = \theta_{i+1} - \theta_{i}$ imagine that the rod si framed with a Bishop frame $\{\vect{u},\vect{v},\vect{t}\}$. Because the material frame $\{\vect{d}_1,\vect{d}_2,\vect{d}_3\}$ is also adapted to the rod centerline, it can be transformed into the Bishop frame with a single rotation of angle $\theta$ around $\vect{d}_3 =\vect{t}$. Because the Bishop frame does not twist around $\vect{d}_3$, the rate of change of angle $\theta$ along the curve directly leads to the computation of the rate of twist as exposed in \cref{eq:d_twist}.
...
\begin{subequations}
	\begin{alignat}{3}
		&Q_{2i+1/2} &&=  GJ_i(\tau_{2i+1/2} - \rconf{\tau}_{2i+1/2})
		\\[0.5em]
		&Q_{2i+3/2} &&=  GJ_i(\tau_{2i+3/2} - \rconf{\tau}_{2i+3/2})
	\end{alignat}
\end{subequations}
...
The shear force are computed directly from \cref{eq:motion_4,eq:motion_5}, assuming that inertial terms are negligible :
\begin{equation}
	\begin{aligned}
	\vect{F}^{\perp} 
	&= (1+\epsilon)^{-1}\vect{d}_3 \times (\vect{M}^{'|xyz} + \vect{m}_{ext})\\
	&= (1+\epsilon)^{-1}\vect{d}_3 \times (\vect{M}^{'|123} + \vect{\varkappa} \times \vect{M} + \vect{m}_{ext})\\
	&= (1+\epsilon)^{-1}\vect{d}_3 \times (\vect{M}^{'|123} + \vect{m}_{ext}) + Q\vect{\kappa b} - \tau \vect{M}^{\perp}
	\end{aligned}
\end{equation}
