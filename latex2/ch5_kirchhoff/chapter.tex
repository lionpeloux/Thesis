...
\begin{figure}[p]
  	\begin{leftfullpage}
		...
		\caption{Description of the motion for a Cosserat rod. This is a typical longitudinal section of a rectangular beam deformed from a reference configuration (bottom) to an actual configuration (top) at time $t$. Cross-sections are defined in the reference configuration to be planar surfaces perpendicular to the beam axis. A material point $\rconf{\vect{p}} \in \rconf{\mathcal{S}}(s)$ is located relatively to the cross-section centroid ($\rconf{\vect{x}}$) thanks to its material coordinates ($X_1$, $X_2$). During the motion, this material point reach a new position $\vect{p} \in \mathcal{S}(s)$. The deformed cross-section is no more planar. The material frame is no more aligned with the beam axis ($\vect{d}_3$ and $\vect{t}$ are not parallel any more). The actual position is measured from the centroid of the deformed cross-section ($\rconf{\vect{x}}$) plus an in-plane component ($X_{\alpha}\vect{d}_{\alpha}$) and a deformation vector ($\vect{u}$). If the cross-sections deform in a rigid-body manner, then $\vect{u}$ is null everywhere.}
...
\end{equation}
In this configuration, we define a cross-section $\mathcal{S}(s)$ as the set of material points lying in a plane perpendicular to $\rconf{\gamma}$ at position $\rconf{\vect{x}}(s)$. By definition, it is a planar surface in the reference configuration. However, there is no evidence that this surface will remain planar in any other configuration. Moreover, for this configuration only, it make sens to choose the centerline as the curve passing through the cross-section centroids.
...
\begin{subequations}
	...
	\end{alignat}
\end{subequations}
The derivative with respect to time is denoted with an overdot symbol.
...
\begin{equation}	
	...
	\norm{\rconf{\vect{x}}'} = 1
\end{equation}
In the deformed configuration, the centerline is still parametrized by $s$ which is no more an arc length parameter because the centerline has suffered stretch. Thus, the unit tangent vector is given by : \footnote{However, because $s_t$ is an arc length parameter of $\gamma_t$ : $\vect{t}(s,t) = \frac{\partial \vect{x}}{\partial s_t}(s,t)$.}
\begin{equation}	
	\vect{t}(s,t) = \frac{{\vect{x}}'(s,t)}{\norm{{\vect{x}}'(s,t)}}
	\quad , \quad
	\norm{\vect{x}'} = \norm{\vect{\eta}'} \neq 1
\end{equation}
We introduce $\epsilon$, the extension of the rod, defined in relation to the axial deformation as :
\begin{equation}	
	 \norm{\vect{\eta}'(s,t)} = \frac{\partial s_t}{\partial s}(s,t) = \Psi'(s,t) = 1 + \epsilon(s,t)
 \end{equation}
 
\subsubsection{Inextensibility}
The rod is said to be inextensible if $\epsilon = 0$ everywhere and at all time. In this case, $s$ is a valid arc length parameter for the centerline in every configurations. Later, we will restrict to the case of rods subject to small extension, that is $\epsilon(t, s) \ll 1$.
...
We consider a \emph{stress-free} configuration of the rod that is called the \emph{reference} configuration.\footnote{See \cite[p.~20]{Audoly2010} for precisions when such a configuration may not exist.} The rod is fully described by its centerline $\rconf{\gamma}$ and its material frame $\{\rconf{\vect{d}_1}, \rconf{\vect{d}_2}, \rconf{\vect{d}_3}\}$.\footnote{We use the notation employed by Antman in his \emph{special Cosserat theory of rods} : \blockcquote[p.~270]{Antman2005}{The motion of a special Cosserat rod is defined by three vector-valued functions : $ [s_1, s_2] \times \mathbb{R} \ni  (s,t) \mapsto \vect{r}(s,t), \; \vect{d}_1 (s,t), \; \vect{d}_2 (s,t) \in \mathbb{E}^3$}. However, somme specific assumptions will be made over the directors in the context of Kirchhoff's theory.} The set of material points lying in a plane perpendicular to $\rconf{\gamma}$ defines a \emph{cross-section} ($\mathcal{S}$) and is thus a planar surface. 
...
\begin{equation}
	\fonctionL{\rconf{\gamma}}{[0,{L}]}{\mathbb{R}^3}{{s}}{\rconf{\vect{x}}({s})}
\end{equation}
As a regular curve, $\rconf{\gamma}$ is $\mathcal{C}^1$ and its tangent vector is continuously defined over [0,L] :
\begin{equation}
	\rconf{\vect{t}} = \frac{d \rconf{\vect{x}}}{d {s}}
\end{equation}
We also require that each cross-section centroid belong to the centerline. We choose $\rconf{\vect{d}_1}({s})$ and $\rconf{\vect{d}_2}({s})$ to be  unit vectors aligned with the principal axes of the cross-section $\mathcal{S}(s)$ so they are perpendicular to each other and lie in the plane of $\mathcal{S}({s})$ :
\begin{equation}
	\fonctionL{(\rconf{\vect{d}_1},\rconf{\vect{d}_2})}{[0,{L}]}{\mathbb{R}^3 \times \mathbb{R}^3}{{s}}{(\rconf{\vect{d}_1}({s}), \rconf{\vect{d}_2}({s}))} 
\end{equation}
...
\end{equation}
Since the material frame is orthonormal, it's evolution along the \emph{undeformed} centerline is described thanks to the \emph{reference strain vector} $\rconf{\vect{\varkappa}}$ defined as :
\begin{equation}
	\frac{d}{d s}\left(\vect{d}_i\right) = \rconf{\vect{\varkappa}}  \times \vect{d}_i
\end{equation}
...
\begin{subequations}
	\begin{alignat}{3}
		&\rconf{\varkappa}_1 &&= \rconf{\kappa}_1 &&= \rconf{\vect{\kappa b}} \cdot \vect{d}_1
		\\
		&\rconf{\varkappa}_2 &&= \rconf{\kappa}_2 &&= \rconf{\vect{\kappa b}} \cdot \vect{d}_2
		\\
		&\rconf{\varkappa}_3 &&= \rconf{\tau} &&= \vect{d}'_1  \cdot \vect{d}_2
	\end{alignat}
\end{subequations}
where $\rconf{\vect{\kappa b}}$ is the curvature binormal vector of $\rconf{\gamma}$ :
\begin{equation}
 	\rconf{\vect{\kappa b}} =  \vect{t} \times  \frac{\partial \vect{t}}{\partial s} = \vect{t} \times \vect{t}'
\end{equation}
$\rconf{\varkappa}_1$ and $\rconf{\varkappa}_2$ are called the \emph{material} or \emph{rod} reference curvatures. $\rconf{\varkappa}_3$ is called the \emph{material} or \emph{rod} reference twist. Note the important distinction with the geometric notions of curvature ($\kappa$) and twist ($\tau_f$) for space curves as defined in \cref{sec:spacecurve}.
...
The curve is still parametrized by $s$, the arc length parameter of the \emph{reference} configuration, as the constitutive laws will be expressed relatively to this configuration. But note that $s$ is no more the arc length parameter of the \emph{deformed} centerline as the rod may have suffer axial extension.\footnote{In Kirchhoff's theory, rods are not supposed to be strictly inextensible but extension has to remain small. Thus, the internal axial force is given by a constitutive law and not considered as a geometric constrained. However, some authors have remarked that it might be convenient and reasonable to solve the equations of motion considering the geometric contraint $\epsilon = 0$. See \cite[p.~98]{Audoly2010} for a detailed discussion of the subject.} Kirchhoff's theory assumes that the material frame remains adapted to the centerline during deformation, or equivalently that transverse shears are neglected.\footnote{This is also known as the \textquote{unsherable} assumption. Indeed, if $\frac{\partial \vect{x}}{\partial s} = \eta_k \vect{d}_k = (1 + \epsilon)\vect{d}_3 \Leftrightarrow \eta_1 = \eta_2 = 0$.} The extension of the centerline is characterized by $\epsilon$ defined such that :
 \begin{equation}
	\frac{\partial \vect{x}}{\partial s} = \eta_3 \vect{d}_3 =  (1 + \epsilon) \vect{d}_3
\end{equation}
...
\begin{subequations}
	...
	\end{alignat}
\end{subequations}
Since the material frame is orthonormal, it's evolution along the \emph{deformed} centerline is described thanks to the \emph{actual strain vector} ${\vect{\varkappa}}$ defined as :
\begin{equation}
	\frac{\partial \vect{d}_k}{\partial s} = {\vect{\varkappa}}  \times \vect{d}_k
\end{equation}
...
\begin{subequations}
	\begin{alignat}{3}
		&{\varkappa}_1 &&= (1+\epsilon) {\kappa_1} &&= (1+\epsilon) \vect{\kappa b} \cdot \vect{d}_1
		\\[0.5em]
		&{\varkappa}_2 &&= (1+\epsilon) {\kappa_2} &&= (1+\epsilon) \vect{\kappa b} \cdot \vect{d}_2
		\\[0.5em]
		&{\varkappa}_3 &&= (1+\epsilon) {\tau} &&= (1+\epsilon) \frac{\partial \vect{d}_1}{\partial s_t}  \cdot \vect{d}_2
	\end{alignat}
\end{subequations}
...
\end{equation}
${\varkappa}_1$ and ${\varkappa}_2$ are called the \emph{material} or \emph{rod} curvatures. ${\varkappa}_3$ is called the \emph{material} or \emph{rod} twist. Again, note the important distinction with the geometric notions of curvature ($\kappa$) and twist ($\tau_f$) for space curves as defined in \cref{sec:spacecurve}. In particular, the material strains ($\varkappa_1$, $\varkappa_2$, $\varkappa_3$) depend on the extension of the rod. These are the strains employed in the classical constitutive laws that lead to the determination of the internal axial force ($N = ES \epsilon$), internal bending moments ($M_1 = EI_1(\varkappa_1-\rconf{\varkappa}_1)$, $M_2 = EI_2(\varkappa_2-\rconf{\varkappa}_2)$) and internal twisting moment ($Q = GJ((\varkappa_3-\rconf{\varkappa}_3)$). 
...
\label{eq:centroid}
	...
	\end{alignat}
\end{subequations}
For a thin slice of the rod ($\delta\mathcal{V}$) between cross-sections $\mathcal{S}(s)$ and $\mathcal{S}(s+ds)$, \cref{eq:inertial_a,eq:inertial_b} yield respectively : \footnote{Indeed, since $\iint_{\mathcal{S}(s)} \vect{r} \;dX_1 dX_2 = \vect{0}$ from \cref{eq:centroid_a} we have $\iint_{\mathcal{S}(s)} \ddot{\vect{r}} \;dX_1 dX_2 =  \iint_{\mathcal{S}(s)} \dot{\vect{\omega}} \times \vect{r} + \vect{\omega} \times ( \vect{\omega} \times \vect{r}) \; dX_1 dX_2 = \vect{0}$ and $\iint_{\mathcal{S}(s)} \vect{r} \times \ddot{\vect{x}} \; dX_1 dX_2 = \vect{0}$ as $\vect{w}$ and $\vect{x}$ are independent of $X_1$ and $X_2$.}
\begin{subequations}
	\begin{alignat}{2}
...
\begin{subequations}
	...
	\\
	\rho (I_1 + I_2)\dot{\omega}_3 +\rho(I_1 - I_1)\omega_1 \omega_2 &\simeq \rho (I_1 + I_2)\dot{\omega}_3
	\end{alignat}
\end{subequations}
...
From now on we consider that the rod material is isotropic and linear elastic.\footnote{This is true at first order for small strains anyway.} This is the framework of the so called \emph{Hookean Elasticity}. This assumption allows the determination of the local displacement field ($\vect{u}$), the strain tensor ($\tens{E}$), the stress tensor ($\tens{S}$) and the constitutive equations that link the axial force ($\vect{F}_1$), the bending moments ($\vect{M}_1$, $\vect{M}_2$) and the twisting moment ($\vect{M}_3$) to the strains ($\epsilon$, $\vect{\varkappa}$, $\rconf{\vect{\varkappa}}$).
...
\end{equation}
where $\lambda$ and $\mu$ are known as the elastic coefficients of Lamé. This coefficients are related to the elastic ($E$) and shear ($G$) modulus and to the Poisson ratio ($\eta$) :
\begin{subequations}
	\begin{alignat}{1}
	\mu &= \frac{E}{2(1+\eta)} = G
	\\[0.5em]
	\lambda &= \frac{2\mu\eta}{1-2\eta}
	\end{alignat}
\end{subequations}
...
\begin{subequations}
	...
	F_3 &= ES\epsilon
	\\
	M_1 &= EI_1(\varkappa_1 - \rconf{\varkappa}_1)
	\\
	M_2 &= EI_2(\varkappa_2 - \rconf{\varkappa}_2)
	\\
