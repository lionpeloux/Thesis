...
We require that ${\gamma}$ is at least a regular space curve. We denote ${s}$ its arc length parameter, ${L}$ its length and $\overbar{\vect{x}}$ the position vector from some fixed origin :
\begin{equation}
...
%\begin{equation}
%	\overbar{\vect{p}}({X_1}, {X_2}, {s}) = 
%	\overbar{\vect{x}}({s}) + {X_1} {\vect{d}_1}({s}) + {X_2} {\vect{d}_2}({s})
%\end{equation}
\begin{subequations}
	\begin{alignat}{2}
...
Let $\vect{\sigma_p}(X_1,X_2,s,t)$ be the \emph{Piola stress vector} at point $\vect{p}(X_1,X_2,s,t) \in \mathcal{S}(s)$ in the \emph{actual} configuration. We call \textquote{right part} the upstream part of the rod between $[s,L]$. Reciprocally, we call \textquote{left part} the downstream part of the rod between $[0,s]$. By definition, the elementary contact force applied from the right part to the left part of the rod on the infinitesimal surface $dX_1dX_2$ around $(X_1,X_2)$ is given by :
\begin{equation}
...
\begin{subequations}
	...
	\end{alignat}
\end{subequations}
%Using \cref{eq:centroid} yields :
%\begin{equation}
%	\iint_{\mathcal{S}(s)} \vect{r} \times \ddot{\vect{p}} \; dX_1 dX_2 = \iint_{\mathcal{S}(s)} \vect{r} \times \ddot{\vect{r}} \; dX_1 dX_2
%\end{equation}
Finally, remark that $\vect{r} \times \ddot{\vect{r}} = (X_1)^2 \vect{d}_1 \times \ddot{\vect{d}_1} + (X_2)^2 \vect{d}_2 \times \ddot{\vect{d}_2} + X_1 X_2 (\vect{d}_1 \times \ddot{\vect{d}_2} + \vect{d}_2 \times \ddot{\vect{d}_1})$. Thus, reminding \cref{eq:sectionprop} :
\begin{subequations}
	\begin{alignat}{2}
