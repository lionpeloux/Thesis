...
We assume the axial force ($\vect{N}$) to vary linearly over $]\vect{x}_{2i},  \vect{x}_{2i+2}[$. The variation occurs if the segment is subject to a distributed load $f_3$, uniform over the segment. Consequently, the axial strain ($1 + \epsilon$) is also required to vary linearly. The value of the axial force and the extension at mid span of each edge are given by :
\begin{subequations}
	\begin{alignat}{1}
	\epsilon_{i+1/2} &= \frac{l_{i}}{\rconf{l}_i} - 1
	\\
	\vect{N_{i+1/2}} &= [ES]_i \epsilon_i \vect{d}_{3,i+1/2}
	\end{alignat}
\end{subequations}
Remark the sign convention : as expected, when edge $\vect{e}_i$ suffers a positive extension, vertex $\vect{x}_i$ \textquote{attracts} vertex $\vect{x}_{i-1}$ to it as $\vect{d}_{3,i+1/2}$ is pointing from $\vect{x}_{i}$ towards $\vect{x}_{i+1}$. Remark also that $\epsilon_{i+1/2} = 0 \Leftrightarrow l_i = \rconf{l}_i$ when the rod is not stretched.
